<template>
    <div>
      ...
      <DateComponent :date="post.date" class="italic" /> <span>~</span> <span>{{ post.category }}</span> <span v-if="post.sub_category">~ {{ post.sub_category }}</span>
      </div>
      <p>{{ post.description }}</p>
      <NuxtLink :to="post.url" class="underline text-secondary" external>check it out</NuxtLink>
    </div>
  </template>
